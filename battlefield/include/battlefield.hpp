...
public:
    ...
    [[eosio::action]] void dbins(name account);
    ...
    [[eosio::action]] void binexttest(binary_extension<name> data);

    [[eosio::action]] void optiontest(optional<name> opt_param);
...
private:
    ...
    struct [[eosio::table]] binaryext_row
    {
        uint64_t id;
        binary_extension<name> binext_field;

        auto primary_key() const { return id; }
    };

    typedef eosio::multi_index<"binaryext"_n, binaryext_row> binaryexts;
